...
export const DiscoveriesQuerySchema = z
  .object({
    ...
    featureSlugs: z.union([z.string(), z.array(z.string())]).optional().transform((v) => (typeof v === "string" ? v.split(",").map((s) => s.trim()).filter(Boolean) : v)).openapi({ description: "Filter by multiple feature slugs (comma-separated string or repeated query param)" }),
    ...
    workflowSlugs: z.union([z.string(), z.array(z.string())]).optional().transform((v) => (typeof v === "string" ? v.split(",").map((s) => s.trim()).filter(Boolean) : v)).openapi({ description: "Filter by multiple workflow slugs (comma-separated string or repeated query param)" }),
...
registry.registerPath({
  ...
  description: "List discoveries with optional filters. Supports filtering by featureDynastySlug (resolves to all versioned slugs via features-service), featureSlugs (comma-separated), featureSlug (exact match), and equivalent workflow filters. Dynasty slug takes priority over slugs list, which takes priority over exact slug.",
  request: {
    headers: IdentityHeadersSchema,
