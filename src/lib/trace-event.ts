export async function traceEvent(
  ...
  if (!url || !apiKey) {
    console.error("[articles-service] RUNS_SERVICE_URL or RUNS_SERVICE_API_KEY not set, skipping trace");
    return;
  }
  try {
    ...
    console.error("[articles-service] Failed to trace event:", err);
  }
}
